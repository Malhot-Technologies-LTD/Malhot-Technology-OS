...
## Status
Nearly done — auth, shell and the test harness are built and CI is green; the owner has bootstrapped the organisation and signed in. Remaining: `lib/permissions.ts`, Vercel/Sentry (owner accounts), Supabase dashboard settings (owner).
...
- [x] RLS integration harness (`tests/fixtures/supabase.ts`, `tests/integration/identity-rls.test.ts`, 13 tests) and pgTAP (`supabase/tests/identity.test.sql`, 15 assertions) — **green in CI** (database job: fresh local stack, migrations from files, pgTAP, types-drift check, RLS tests)
...
- [ ] Repo secrets `E2E_EMAIL`/`E2E_PASSWORD` (a dedicated test member, not the owner) so the account e2e journeys run in CI; repo variables for the Supabase URL/publishable key are already set
...
- CI is green on `main` (2026-09-16). Migration files apply cleanly from scratch, which also validates the renamed versions.
- Next step on resume: `lib/permissions.ts` (`can()` + matrix tests from `docs/product/user-roles.md`) as the bridge into Phase 3; Vercel project + Sentry when the owner is ready; then Phase 2 (website) or Phase 3 (projects) per the owner's priority.
